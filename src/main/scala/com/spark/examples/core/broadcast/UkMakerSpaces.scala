package com.spark.examples.core.broadcast
...
object UkMakerSpaces {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("UkMakerSpaces").
      setMaster("local[1]")
    val sparkContext = new SparkContext(conf)

    val postCodeMap = sparkContext.broadcast(loadPostCodeMap())

    val makerSpaceRdd = sparkContext.textFile("/apps/hostpath/datasets/uk-makerspaces-identifiable-data.csv")
  ...
  def loadPostCodeMap(): Map[String, String] = {
    ...
    Source.fromFile("/apps/hostpath/datasets/uk-postcode.csv").getLines.map(line => {
      val splits = line.split(Utils.COMMA_DELIMITER, -1)
      splits(0) -> splits(7)
    }).toMap
